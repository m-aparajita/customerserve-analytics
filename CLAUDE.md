...
| Voice ("Ask Aloud") | `agent/voice.py` — STT via Groq `whisper-large-v3-turbo`; `app.py::voice_respond()` — TTS via browser `speechSynthesis`, also powers the "Listen to answer" button; mic feeds transcribed text into the same `respond()` pipeline as typed input |
...
- **Voice & TTS:** STT via Groq `whisper-large-v3-turbo` (reuses `GROQ_API_KEY`; free tier 2,000 req/day, 7,200 audio-sec/hr) — transcribed text is not a separate trust boundary, same guardrails/RBAC as typed input. TTS is the browser's built-in `speechSynthesis` (client-side JS, zero new deps, avoids Groq's paid per-character TTS) — powers both the mic path and the "Listen to answer" button, which reads ChartAgent's insight bullets aloud, dropping a trailing bullet if it's a follow-up question. Mic widget: `editable=False`, download/share hidden. **Client-only `fn=None`+`js=` handlers (like Listen to answer) must read hidden `gr.Textbox` inputs, never `gr.State`** — State lives server-side only and never syncs to the browser, so a JS-only handler reading it always gets a stale/empty value. Caused Listen-to-answer's silent no-op in prod until `current_insights`/`current_answer` moved from `State` to `Textbox`.
