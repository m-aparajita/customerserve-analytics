...
- **"Listen to answer" button:** `listen_btn.click(fn=None, js=...)` — pure client-side `speechSynthesis`, no server round-trip. Reads the ChartAgent insight bullets, dropping a trailing bullet if it's a follow-up question (the deep-mode "exploration hook"); falls back to the plain text answer when there are no insights. Its two inputs (`current_insights`, `current_answer`) **must be hidden `gr.Textbox`, not `gr.State`** — `gr.State` values live server-side only and are never mirrored into the browser, so a `fn=None`+`js=` handler reading a State input always gets its stale initial value, not what `respond()` last set. This caused a silent no-op in prod (confirmed via console: both args logged empty) until switched to Textbox. Same trap applies to any future client-only JS handler.
