...
- **System prompt:** embeds a compact schema listing (`mcp/tools.py::get_schema_compact()` — one line per table, `table(col type, col type, ...)`, no row counts, no JSON indentation) directly in `agent/system_prompt.py::build()`. **Reversed 2026-07** from the prior "keep system prompt short, always call `get_schema` first" decision — free-tier TPM dropped from 30K to 8K on the post-scout models, and the mandatory per-turn `get_schema` round trip (plus its indent=2 JSON resent every subsequent round) was the single biggest avoidable cost. `get_schema` tool is still available as a fallback if the model suspects the embedded schema is stale or needs row counts, just no longer mandatory.
- **TPM budget management (2026-07, free tier dropped 30K→8K):** `mcp/tools.py::cap_rows_for_llm()` caps any tool result's `rows` list to 40 before it re-enters LLM context (both agents) — the RBAC row limits (up to 10,000 for admins) exist for charting/export, not for what the model needs to see. Full uncapped data is captured separately (`last_query_result` in QueryAgent, `rows`/`chart_json` in ChartAgent) before truncation. `ChartAgent.analyze()` also replaces `build_chart`'s full `fig.to_json()` tool result with a minimal `{"status": "chart created", ...}` confirmation — the model never needs the figure spec back, and it was being resent every subsequent round. QueryAgent's history window (`_HISTORY_TURNS`) dropped 6→3 turns, with each stored assistant answer capped to 600 chars (`_HISTORY_ANSWER_CHARS`).
