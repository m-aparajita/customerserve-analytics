...
**TTS ("Listen to answer"):** the browser's built-in `speechSynthesis` API, entirely client-side JS — zero API calls, zero new dependencies. Deliberately avoids Groq's paid per-character TTS endpoint. The button reads ChartAgent's insight bullets aloud, dropping the last bullet when it's phrased as a follow-up question (the deep-mode "exploration hook"), and falls back to the plain text answer when a query produced no insights.

**A second production bug, same root cause pattern as the `ffmpeg` one above — a client-side effect silently doing nothing:** the button's Python-side click handler is `fn=None, js=...` — a pure client-side call with no server round-trip, so `speechSynthesis` never touches the Groq API or the Python backend at all. Its two text inputs were originally wired to `gr.State`, which seemed natural since they already held "current answer" data used elsewhere. But `gr.State` lives server-side only; Gradio never mirrors its value into the browser. A `js=`-only handler reading a `State` input therefore always receives that component's stale initial value, not what the last `respond()` call actually set — so the button fired, `speechSynthesis` ran, but on an empty string. Diagnosed by adding `console.log` inside the JS itself (the only vantage point that could see the actual argument values) and confirming both inputs logged empty on click. Fixed by switching `current_insights`/`current_answer` to hidden `gr.Textbox` components — real, rendered elements whose values Gradio does sync to the client. The general lesson: any future client-only (`fn=None`+`js=`) handler needs its inputs on real components, not `State`.
...
>
> A related one: the "Listen to answer" button went live and simply did nothing — no error, no sound. Same shape of bug as the `ffmpeg` one: a client-side failure invisible to any server-side log, because the handler (`fn=None`+`js=`) never talks to the server at all. I added a `console.log` inside the JS itself, since that was the only place that could see what the handler actually received, and it showed both text inputs arriving empty. They were wired to `gr.State`, which never syncs to the browser — a JS-only handler reading `State` always sees its stale initial value. Switching those two inputs to hidden `gr.Textbox` (a real, client-synced component) fixed it. The pattern across both bugs: when a feature fails with zero error output, suspect a boundary the normal logging can't see across — a missing native dependency, or a value that never left the server in the first place.
